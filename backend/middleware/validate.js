// ─── PATCH validate.js ───────────────────────────────────────────────────
// Remplace les deux schemas concernés dans le fichier validate.js existant.
// Changement : ajout de "wave" et "orange_money" dans paymentMethod

// 1. Remplace createOrder.paymentMethod :
// AVANT :
//   paymentMethod: Joi.string().valid("cash", "card", "mobile").required(),
// APRÈS :
//   paymentMethod: Joi.string().valid("cash", "card", "mobile", "wave", "orange_money").required(),

// 2. Remplace updatePayment.paymentMethod :
// AVANT :
//   paymentMethod: Joi.string().valid("cash", "card", "mobile").optional(),
// APRÈS :
//   paymentMethod: Joi.string().valid("cash", "card", "mobile", "wave", "orange_money").optional(),

// ─── Fichier complet corrigé ──────────────────────────────────────────────
"use strict";
const Joi = require("joi");

const PAYMENT_METHODS = ["cash", "card", "mobile", "wave", "orange_money"];
...
const schemas = {
  ...
  createOrder: Joi.object({
    ...
    paymentMethod: Joi.string().valid(...PAYMENT_METHODS).required(), // ✅ wave + om
    paymentStatus: Joi.string().valid("pending", "paid").optional(),  // ✅ accepte paid dès la création
  ...
  updatePayment: Joi.object({
    ...
    paymentMethod: Joi.string().valid(...PAYMENT_METHODS).optional(), // ✅ wave + om
  }),
