...
class ReservationManager {
  ...
  async _setStatus(id, status, extra = {}) {
    try {
      ...
      this._render();
      NotificationManager?.showSuccess(id, status === "confirmed" ? "Confirmée ✅" : "Mise à jour", `Réservation ${id}`, 2500);
    } catch {
      NotificationManager?.showSuccess(id, "Erreur", "Impossible de mettre à jour", 3000);
    }
  }
  ...
  async _updateReservation(id, data) {
    try {
      ...
      this._render();
      NotificationManager?.showSuccess(id, "Enregistré", "Réservation mise à jour", 2000);
    } catch {
      NotificationManager?.showSuccess(id, "Erreur", "Impossible de sauvegarder", 3000);
    }
  }
